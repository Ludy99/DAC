...
void KeyGPIO_Configuration(void)
{
	...
	*/
	...
	if(	Buffer[8]>=40)
	{
		...
		TimeH_Correct[5]=	Buffer[8];
	break;  
	
		case 0x26:										//跳闸时间设置
          data=MAKETIME(Buffer[8],Buffer[7]);
          Time_T[0]=data;                              //Time_T[0]-Time_T[5]分别为ABCDEF各相跳闸时间
          Time_T[1]=data;
          Time_T[2]=data;
          Time_T[3]=data;
          Time_T[4]=data;
	 ...
	 if(Buffer[2]==0x30)									//开机上电以后收到的恢复现场（加了校正值）
	 {
        ...
        if(Buffer[50])							 //断路器失灵
		  ...
		  CH_Operation(Current_H);
 	  	  Hertz=MAKETIME(Buffer[12],Buffer[11]);	  //跳闸电流
		  HZ_Operation(Hertz);
		  Rad=MAKETIME(Buffer[14],Buffer[13]);	  //合闸电流
		  RAD_Operation(Rad);			  	 	 
	 }
  }       
}
...
//			DMA1_Channel2->CNDTR=NbrOfDataToSend; //DMA1传输数据量
//			DMA1_Channel2->CMAR=(u32)TxBuffer;
...
void Hertz_Operation(uint8_t Num)
{
 ...
 { 
   case 0:	break;
   case 1:	Send_Series_Data(&Hertz1[0],8); Hertz_Handle(1);  Hertz=20; break;
   case	2:	Send_Series_Data(&Hertz2[0],8); Hertz_Handle(2);  Hertz=50; break;
   case	3:	Send_Series_Data(&Hertz3[0],8);	Hertz_Handle(3);  Hertz=100; break;
   case	4:  Send_Series_Data(&Hertz4[0],8); Hertz_Handle(4);  Hertz=150; break;
   case	5:	Send_Series_Data(&Hertz5[0],8); Hertz_Handle(5);  Hertz=500; break;
   case	6:  Send_Series_Data(&Hertz6[0],8); Hertz_Handle(6);  Hertz=1000; break; 
   default: break; 
 }
}
...
void HZ_Operation(uint16_t Num)
{
 switch(Num)
 {
   case 20:	    Hertz_Handle(1);  break;
   case 50:     Hertz_Handle(2);  break;
   case	100:	Hertz_Handle(3);  break;
   case	150:	Hertz_Handle(4);  break;
   case	500: 	Hertz_Handle(5);  break;
   case	1000:	Hertz_Handle(6);  break;
   default: break; 
 }
}
void RAD_Operation(uint16_t Num)
{
 switch(Num)
 {
   case 30:	    CurrentH_Handle(1);  break;
   case 45:     CurrentH_Handle(2);  break;
   case	60:	    CurrentH_Handle(3);  break;
   case	90:	    CurrentH_Handle(4);  break;
   case	180: 	CurrentH_Handle(5);  break;
   case	270:	CurrentH_Handle(6);  break;
   default: break; 
 }
}
void Save(uint8_t*TxBuffer_Save,uint8_t NbrOfDataToSend)
