...
//DMA配置
//void SineWave_DMA_Config(DMA_Channel_TypeDef* DMA_CHx,u32 cpar,u32 cmar,u16 cndtr)
//{                  
...
//    DMA_InitStructure.DMA_BufferSize = cndtr;//寄存器大小
...
//	DMA_InitStructure.DMA_PeripheralBaseAddr = cpar;//外设地址为DAC通道1的地址
//    DMA_InitStructure.DMA_MemoryBaseAddr = cmar;//波形数据表内存地址
...
//	SineWave_Data( 256 ,SineWave_Value);     //生成输出正弦波的波形表
	...
	SineWave_DMA_Config();
//    SineWave_DMA_Config(DMA1_Channel3,(u32)&USART1->DR,
//(u32)SendBuff,SEND_BUF_SIZE);              //配置DMA
